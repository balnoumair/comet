//! Session-document size and cadence constants.

/// Max bytes for a single message entry before continuation splitting.
pub const MSG_INLINE_MAX: usize = 256 * 1024;
/// History retention window for local snapshot trimming (days).
pub const RETAIN_DAYS: u32 = 3;
/// Fold the update log into the local snapshot at this size (lossless).
...
/// Reserved cadence for persisted update batching (ms).
...
/// Number of trailing messages retained for a compact transcript tail.
